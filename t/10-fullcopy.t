...
use bucordoTesting;
my $bct = bucordoTesting->new({sync => 'fctest', location => 'fullcopy'})
...
$res = $bct->ctl('bucordo add dbgroup pg A:source B:fullcopy C:fullcopy D:fullcopy');
like ($res, qr/Created dbgroup "pg"/, $t);

## Create a new sync
$t = q{Created a new sync};
$res = $bct->ctl('bucordo add sync fctest herd=all dbs=pg');
like ($res, qr/Added sync "fctest"/, $t);

## Create a new sync for the sequences only
$t = q{Created a new sync};
$res = $bct->ctl('bucordo add sync seqtest herd=seqonly dbs=A:source,B:Source,C:target,D:target');
...
$dbhA->do('alter sequence bucordo_test_seq1 start 20 restart 25 minvalue 10 maxvalue 100');
$dbhA->commit();

## Kick off the sync, then check that everything was replaced
$bct->ctl('bucordo kick fctest seqtest 0');
sleep(3);
$bct->check_for_row([[2]], [qw/ B C D/]);
...
$bct->ctl('bucordo kick seqtest 0');
