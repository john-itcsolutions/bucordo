...
$res = $bct->ctl("bucordo add db A2 dbname=bucordo_test user=$dbuserA port=$dbportA host=$dbhostA");
like ($res, qr/Added database "A2"/, $t);

$t = 'Add database works for cluster B works with ssp=false';
$res = $bct->ctl("bucordo add db B dbname=bucordo_test user=$dbuserB port=$dbportB host=$dbhostB ssp=0");
like ($res, qr/Added database "B"/, $t);

$t = 'List databases gives expected results';
$res = $bct->ctl('bucordo list databases');
my $statA = qq{Database: A\\s+Status: active\\s+Conn: psql -p $dbportA -U $dbuserA -d bucordo_test -h $dbhostA};
my $statA2 = qq{Database: A2\\s+Status: active\\s+Conn: psql -p $dbportA -U $dbuserA -d bucordo_test -h $dbhostA};
my $statB = qq{Database: B\\s+Status: active\\s+Conn: psql -p $dbportB -U $dbuserB -d bucordo_test -h $dbhostB \\(SSP is off\\)};
...
$res = $bct->ctl('bucordo remove db A B');
...
$res = $bct->ctl("bucordo add db A dbname=bucordo_test user=$dbuserA port=$dbportA host=$dbhostA group=group1");
...
$res = $bct->ctl("bucordo add db B dbname=bucordo_test user=$dbuserB port=$dbportB host=$dbhostB group=group1");
...
$bct->ctl('bucordo remove db B');
$res = $bct->ctl("bucordo add db B dbname=bucordo_test user=$dbuserB port=$dbportB host=$dbhostB group=group1:source");
like ($res, qr/Added database "B" to dbgroup "group1" as source/s, $t);

$t = q{Adding a database into a new group works with 'dbgroup'};
$bct->ctl('bucordo remove db B');
$res = $bct->ctl("bucordo add db B dbname=bucordo_test user=$dbuserB port=$dbportB host=$dbhostB dbgroup=group1:replica");
like ($res, qr/Added database "B" to dbgroup "group1" as target/s, $t);

## Tests for 'remove database'

$t = q{Remove database gives expected message when database does not exist};
$res = $bct->ctl('bucordo remove db foobar');
like ($res, qr/No such database "foobar"/, $t);

$t = q{Remove database works};
$res = $bct->ctl('bucordo remove db B');
...
$res = $bct->ctl('bucordo remove db A A2 B foo');
...
$res = $bct->ctl('bucordo list db');
like ($res, qr/No databases/, $t);

$bct->ctl("bucordo add db B dbname=bucordo_test user=$dbuserB port=$dbportB host=$dbhostB ssp=1");
$t = q{List databases shows the server_side_prepare setting};
$res = $bct->ctl('bucordo list database B -vv');
...
$res = $bct->ctl('bucordo update db A port=12345 user=bob');
like ($res, qr/Changed bucordo.db dbport from \d+ to 1234/, $t);

$t = 'Update database works when adding to a new group';
$res = $bct->ctl('bucordo update db A group=group5');
...
$res = $bct->ctl('bucordo update db B group=group2:replica');
...
$res = $bct->ctl('bucordo update db A status=inactive DBport=12345');
like ($res, qr/No change needed for dbport.*Changed bucordo.db status from active to inactive/s, $t);

$t = q{List database returns correct information};
$res = $bct->ctl('bucordo list dbs');
like ($res, qr/Database: A.*Status: inactive.*Database: B.*Status: active/s, $t);

$t = q{Remove database works};
$res = $bct->ctl('bucordo remove db A B --force');
