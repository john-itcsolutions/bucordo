...
use bucordoTesting;
my $bct = bucordoTesting->new({location => 'conflict'})
...
$bct->ctl('bucordo kick sync ctest 10');
...
$res = $bct->ctl('bucordo update sync ctest conflict="a b c"');
...
$res = $bct->ctl('bucordo update sync ctest conflict="C A B"');
like($res, qr{Set conflict strategy}, $t);

$bct->ctl('bucordo reload sync ctest');
...
$t = q{Set conflict strategy to 'bucordo_latest'};
$res = $bct->ctl('bucordo update sync ctest conflict=latest');
like($res, qr{Set conflict strategy to 'bucordo_latest'}, $t);

$t = q{Reloaded sync ctest};
$res = $bct->ctl('bucordo reload sync ctest');
...
## Test of conflict handler 'bucordo_latest_all_tables'
$t = q{Set conflict strategy to 'bucordo_latest_all_tables'};
$res = $bct->ctl('bucordo update sync ctest conflict=latest_all');
like($res, qr{Set conflict strategy to 'bucordo_latest_all_tables'}, $t);
...
$res = $bct->ctl('bucordo add customcode ctest1 whenrun=conflict sync=ctest src_code=t/ctest1.pl');
...
$res = $bct->ctl('bucordo add customcode ctest2 whenrun=conflict relation=bucordo_test1 src_code=t/ctest2.pl');
